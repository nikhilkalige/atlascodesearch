...
class CindexCommand(sublime_plugin.WindowCommand, _CindexListener):
    ...
    def run(self, index_project=False):
        ...
        self.window.active_view().set_status('AtlasCodeSearch',
                                             'cindex (starting)')
        self._total_indexed = 0

        try:
            s = settings.get_project_settings(self.window.folders(), index_project)
    ...
    def _increment_total_indexed(self, count):
        ...
        self.window.active_view().set_status(
            'AtlasCodeSearch', 'cindex ({} files)'.format(
                self._total_indexed))
